...
ldpd::LdpRawTlv* parse_and_writeback_tlv(const ldpd::LdpRawTlv *parsed_raw_tlv) {
    ...
    switch (parsed_val->getType()) {
        ...
        case LDP_TLVTYPE_COMMON_SESSION: {
            ldpd::LdpCommonSessionParamsTlvValue *casted = (ldpd::LdpCommonSessionParamsTlvValue *) parsed_val;

            printf("====== val type is comm-s-params.\n");
            printf("====== proto-ver: %u.\n", casted->getProtocolVersion());
            printf("====== keepalive: %u.\n", casted->getKeepaliveTime());
            printf("====== pvlim: %u.\n", casted->getPathVectorLimit());
            printf("====== max pdu len: %u.\n", casted->getMaxPduLength());
            printf("====== r-rid: %s.\n", casted->getReceiverRouterIdString());
            printf("====== r-lbl-space: %u.\n", casted->getReceiverLabelSpace());

            ldpd::LdpCommonSessionParamsTlvValue *writeback = new ldpd::LdpCommonSessionParamsTlvValue();

            writeback->setKeepaliveTime(casted->getKeepaliveTime());
            writeback->setPathVectorLimit(casted->getPathVectorLimit());
            writeback->setMaxPduLength(casted->getMaxPduLength());
            writeback->setReceiverRouterId(casted->getReceiverRouterId());
            writeback->setReceiverLabelSpace(casted->getReceiverLabelSpace());
...
#define INIT_PDU "\x00\x01\x00\x20\x0a\x00\x01\x01\x00\x00\x02\x00\x00\x16\x00\x00\x00\x02\x05\x00\x00\x0e\x00\x01\x00\xb4\x00\x00\x00\x00\x0a\x00\x00\x06\x00\x00"
